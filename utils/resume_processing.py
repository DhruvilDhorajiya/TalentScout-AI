...
def analyze_resume_consistency(resume_text, candidate_info):
    ...
    if years_found:
        ...
        if abs(max_years - claimed_years) > 2:
            ...
            findings.append(
                f"Experience discrepancy: Claimed {claimed_years} years, Resume suggests {max_years} years"
            )

    # Check skills match
    claimed_skills = set(
        skill.lower() for skill in candidate_info.get("Tech Stack", [])
    )
    found_skills = set()
    ...
    if missing_skills:
        ...
        findings.append(
            f"Skills mentioned but not found in resume: {', '.join(missing_skills)}"
        )
    ...
    position_match = any(
        word in resume_text.lower() for word in position_words if len(word) > 3
    )
